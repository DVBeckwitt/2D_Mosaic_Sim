...
- Renders each matrix cell as a transparent visual sphere sprite plus a Bragg-only anchor sprite, crops the visual sprite to visible pixels, scales and centers from the Bragg footprint inside that crop, clips helper geometry to the cell, and composites the final PNG on one 2D canvas.
...
- For the Bragg-bbox sizing fix, headless Edge/CDP reached the Dash page and found the `Save 3x3 Matrix` button, but Dash headless mode did not expose `window.Plotly` before graph render in that harness; an app-served Plotly-bundle injection attempt timed out before export. A Kaleido numeric smoke for all anchor figures also timed out. Treat browser visual re-check as the remaining manual verification item before release if the local headless setup remains unstable.
...
No migration is required. The change is additive for users: no existing mode, CLI argument, console script, public Python function signature, Dash component ID, or trace name was removed. The private matrix figure helper no longer accepts a camera override because the matrix export intentionally uses the reference view. The export spec now includes private `bragg_anchor_figure`, `bragg_cell_fill_fraction`, and `preserve_relative_l_scale` fields for browser composition. The old subplot matrix builder is no longer the active save path, but it remains private code/test support. The 2026-05-21 cleanup did not change public or private call signatures. Existing users can ignore the new button and keep the previous workflow.
...
The browser now renders and crops nine full visual Plotly 3D sprite figures plus nine Bragg-only anchor figures before composing the final canvas. Keep monitoring export latency and browser memory use if the matrix dimensions, render size, or trace density increase.

Code review on 2026-05-21 found no avoidable new files, no new dependencies, no obvious security issue, and no required correctness changes. The Bragg-bbox sizing fix modifies existing source/tests/docs only and derives anchor figures from already-built sprite JSON to avoid unnecessary Plotly figure construction. Remaining optional cleanup is to remove the private subplot matrix helper if the debug/test fallback is no longer useful, and to replace substring-only JavaScript callback assertions with an automated parse or browser smoke in CI if browser export regressions become common.
