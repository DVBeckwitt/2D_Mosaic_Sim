...
## Status
Ready for local use as of 2026-05-20. Updated on 2026-05-21 so the active save path is content-driven sprite composition instead of nine 3D subplots. Merged into local `main` with the already-shipped mosaic UI bandwidth work present, cleaned up duplicate helper/test code without changing behavior, fixed the sprite-composite matrix sizing so each cell scales and centers from the Bragg footprint, then removed the unused private Bragg anchor export payload.
...
- Renders each matrix cell as a transparent visual sphere sprite, crops the visual sprite to visible pixels, measures the centered non-cyan Bragg footprint inside that crop, scales and centers from that Bragg footprint, clips helper geometry to the cell, and composites the final PNG on one 2D canvas.
...
- 2026-05-21 Bragg-anchor payload cleanup gate passed: `python -m pytest tests/test_unified_app.py -k "special_cause_matrix_export_spec or special_cause_matrix_export_clientside_callback" -q`, `python -m compileall -q mosaic_sim specular_reflection_sim.py`, `python -m pip check`, `python -m build`, and `git diff --check`.
...
No migration is required. The change is additive for users: no existing mode, CLI argument, console script, public Python function signature, Dash component ID, or trace name was removed. The private matrix figure helper no longer accepts a camera override because the matrix export intentionally uses the reference view. The export spec includes private `bragg_cell_fill_fraction` and `preserve_relative_l_scale` fields for browser composition. The unused private `bragg_anchor_figure` field was removed in follow-up cleanup because the browser measures the Bragg footprint from the visual sprite image. The old subplot matrix builder is no longer the active save path, but it remains private code/test support. Existing users can ignore the new button and keep the previous workflow.
