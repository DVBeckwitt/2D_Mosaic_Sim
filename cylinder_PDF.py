...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.RangeSlider(id='range-slider', min=0, max=1, step=0.01,
    ...
    html.Div([
        ...
        html.Label("σ_φ:"), dcc.Input(id='sigmaPhi-input', type='number', value=0.2, step=0.1),
              ...
              'alignItems': 'center'}),
    html.Div([
        html.Button("Original Only", id='orig-btn',  n_clicks=0),
        html.Button("Single Rod",    id='single-btn', n_clicks=0),
        html.Button("Reset Cloud",   id='reset-btn',  n_clicks=0),
    ], style={'padding': '10px', 'display': 'flex', 'gap': '10px'}),
])
...
def update_figure(int_range, x0, y0, sigma_I, sigma_phi,
                  n_orig, n_single, n_reset):
    # ---- fixed sampling ----
    z_min, z_max = -3.0, 3.0
    ...
    # ---- geometry basis ----
    r = np.hypot(x0, y0)
    if r == 0:                          # on‑axis: choose a reference radial direction
        e_r = np.array([1.0, 0.0, 0.0])
    else:
        e_r = np.array([x0, y0, 0.0]) / r
    e_z = np.array([0.0, 0.0, 1.0])
    k_axis = np.cross(e_r, e_z); k_axis /= np.linalg.norm(k_axis)
    ...
    xs, ys, zs_all, cs = [], [], [], []
    xs0, ys0, zs0, cs0 = [], [], [], []   # ψ = 0 only
    xs_s, ys_s, zs_s, cs_s = [], [], [], []  # single vertical rod

    for phi in phis:
        w_phi = p_phi(phi, sigma_phi) * dphi
        v_phi = rodrigues(e_r * r, k_axis, phi)           # foot  (zero if r==0)
        f_phi = rodrigues(e_z,      k_axis, phi)          # rod direction
        ...
        for i, psi in enumerate(psis):
            ...
            w = Ivals * (dpsi / (2*np.pi))                # intensity weight

            xs.extend(x_rot); ys.extend(y_rot); zs_all.extend(base_pts[:, 2]); cs.extend(w)
            if i == 0:                                    # store the ψ = 0 subset
                xs0.extend(x_rot); ys0.extend(y_rot); zs0.extend(base_pts[:, 2]); cs0.extend(w)

    # single unrotated vertical rod (no φ, ψ)
    for z in zs:
        xs_s.append(x0); ys_s.append(y0); zs_s.append(z); cs_s.append(I0(z, sigma_I))

    # ---- to arrays + colour normalisation ----
