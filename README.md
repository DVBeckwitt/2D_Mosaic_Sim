...
- Feature: special-cause reciprocal rendering keeps Bragg/Ewald surfaces transparent while the sampled green Bragg/Ewald intersection lines render fully opaque. It uses an RGB Bragg colorscale so Plotly opacity is controlled in one place.
...
- Bug/error status: fixed the non-zero special-cause bandwidth bug where changing λ bandwidth moved the Ewald center instead of preserving the central Ewald center and varying only the Ewald radius. No current application runtime error is known for this change.
...
- Shipping status: local `pip check`, special-cause detector regression tests, full test suite, compile, package build, code review, and diff whitespace checks pass for this update; rollback is a normal git revert of this bug-fix commit.
