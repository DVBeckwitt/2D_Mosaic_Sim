...
- Feature: special-cause reciprocal rendering now keeps the Bragg sphere mosaic map prominent while leaving auxiliary Ewald shell surfaces and the continuous Bragg/Ewald overlap band transparent. The sampled green Bragg/Ewald intersection lines remain fully opaque, and the Bragg sphere keeps an RGB mosaic colorscale with a visible colorbar.
- Browser/runtime status: prior local Dash/Chromium verification confirmed `Special Cause Reciprocal` at `λ bandwidth = 5.00%` renders seven live green `Bragg/Ewald overlap` line traces, one overlap band, and the inner/outer shell traces with no failed requests and no HTTP `>=400` responses. The fixed-center bandwidth and shell sample-count behavior are covered by figure-level regression tests for the shell center, overlap band, sampled overlap lines, and unified GUI control/state path.
- Bug/error status: fixed the special-cause rendering regression where the Bragg sphere mosaic became hard to see because it shared the same transparent opacity as the Ewald shell and overlap-band helper surfaces. The earlier non-zero bandwidth fix, which keeps the Ewald center fixed and varies only the Ewald radius, remains covered by regression tests.
- Migration status: no deprecation or migration is required for this bug fix. No existing mode, CLI argument, console entry point, public Python function signature, public Dash component ID, or trace name was removed. Existing detector/fibrous scripts keep their previous behavior unless `--wavelength-bandwidth-pct` is supplied. Direct callers of `build_special_cause_reciprocal_figure()` still use the special-case `(0,0,3)` / `5%` default; pass `L=12, wavelength_bandwidth_pct=0.0` for the old direct-call default view.
- CI status: `.github/workflows/ci.yml` runs editable install, `pip check`, compile, tests, and package build on pull requests and pushes to `main` across Python 3.11 and 3.13.
- Shipping status: local `pip check`, special-cause detector regression tests, unified special-cause regression tests, full test suite, compile, package build, code review, and diff whitespace checks pass for this update; rollback is a normal git revert of this bug-fix commit.
