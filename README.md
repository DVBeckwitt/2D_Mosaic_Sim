...
- Feature: `Special Cause Reciprocal` can export a single 3x3 matrix PNG from the current camera and current settings. The matrix columns use `theta_i = 5°, 10°, 15°`; rows use `003`, `006`, and `009`; all panels share one mosaic-intensity color legend.
...
- Shipping status: local `pip check`, compile, full test suite, package build, code review, and diff whitespace checks pass for this update; rollback is a normal git revert of this feature commit.
