...
- Shipping status: local `pip check`, compile, full test suite, package build, code review, and diff whitespace checks pass for the original matrix export merge. The 2026-05-21 Bragg-bbox sizing fix passed export-focused special-cause tests on Python 3.13, broader special-cause matrix tests on Python 3.11, compile, `git diff --check`, and a headless Edge export smoke with image-footprint analysis. The Bragg-anchor callback restoration passed focused and broader special-cause matrix tests, compile, `pip check`, package build, `git diff --check`, and a headless Edge/CDP export smoke that downloaded a fresh `1800 x 1800` matrix PNG; full `pytest` currently aborts in unrelated Plotly-heavy cylinder/fibrous tests with a Windows access violation on the local Python 3.13 environment. Rollback is a normal git revert of the matrix export feature, cleanup, Bragg-bbox fix, or Bragg-anchor callback commit. See `docs/launch/2026-05-13-special-cause-matrix-export.md`.
