...
- Feature: `Special Cause Reciprocal` can export a single 3x3 matrix PNG from the current camera and current settings. The matrix columns use `theta_i = 5°, 10°, 15°`; rows use `003`, `006`, and `009` with left-side `L = 3`, `L = 6`, and `L = 9` labels; all panels share one mosaic-intensity color legend.
